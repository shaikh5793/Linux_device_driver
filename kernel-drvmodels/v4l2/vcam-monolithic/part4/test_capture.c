 ...
 *
 * test_capture.c - Userspace MMAP capture test for vcam_ring
 */
...
static int find_device(void)
{
	...
	for (i = 0; i < 10; i++) {
		...
		if (ioctl(fd, VIDIOC_QUERYCAP, &cap) == 0 &&
		    strcmp((char *)cap.driver, "vcam_ring") == 0) {
			printf("Found vcam_ring at %s\n", path);
			return fd;
		}
		close(fd);
	}
...
int main(void)
{
	...
	printf("=== vcam_ring Capture Test ===\n\n");

	fd = find_device();
	if (fd < 0) {
		printf("FAIL: vcam_ring device not found\n");
		return 1;
	}
