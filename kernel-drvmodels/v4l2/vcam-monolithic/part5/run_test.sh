#!/bin/bash
# Part 5: IRQ + Work Queue — Automated Test with Feature Demonstration
...
cleanup() {
    echo ""
    echo "--- Cleanup ---"
    sudo rmmod vcam_irq 2>/dev/null || true
...
echo "============================================"
echo " Part 5: IRQ + Work Queue"
echo " NEW CONCEPT: Interrupt-driven frame completion with request_irq + workqueue"
echo "============================================"
echo ""
...
echo -e "${CYAN}=== Feature Demonstration: Interrupt-Driven Frame Completion ===${NC}"
log_info "Part 4 polled for completion; Part 5 uses request_irq + workqueue"
echo ""
echo "  -- /proc/interrupts (vcam) --"
cat /proc/interrupts | grep -i vcam 2>/dev/null || log_info "No vcam IRQ line found in /proc/interrupts"
echo ""
if command -v v4l2-ctl &>/dev/null; then
    VDEV=$(find_vcam_dev "vcam_irq")
    if [ -n "$VDEV" ]; then
        log_info "Found vcam_irq device at $VDEV"
        echo ""
        echo "  -- v4l2-ctl --all --"
        v4l2-ctl -d "$VDEV" --all 2>&1 | head -30
    else
        log_info "Could not find vcam_irq device via v4l2-ctl"
    fi
else
...
sudo dmesg | grep -i "vcam_irq" | tail -15
echo ""
