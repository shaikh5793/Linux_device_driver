 ...
 *   - ISR top-half: read + acknowledge interrupt, schedule deferred work
 *   - IRQ bottom-half via workqueue (cannot call vb2_buffer_done in hardirq)
 *   - Replacing polled completion with hardware notification
 *
...
/*
 * vcam_isr -- Hardware interrupt service routine (top half)
 *
 * Called in hardirq context when the VCAM-2000 fires an interrupt.
 * The ISR must be fast -- it only reads/acknowledges the interrupt
 * and schedules the bottom half.  No sleeping, no vb2 calls.
 *
 * Real camera ISR pattern:
 *   1. Read INT_STATUS to identify the interrupt source
 *   2. Write back to INT_STATUS to acknowledge (W1C = write-1-to-clear)
 *   3. Schedule deferred work for heavy processing
 *   4. Return IRQ_HANDLED (or IRQ_NONE if not our interrupt)
 */
...
/*
 * vcam_irq_work -- IRQ bottom half (deferred processing)
 *
 * Runs in process context (workqueue), so it can sleep and call
 * vb2_buffer_done().  This is where completed frames are returned
 * to the VB2 framework.
 *
 * The algorithm:
 *   1. Read hardware TAIL pointer (how far hardware has progressed)
 *   2. Walk from driver's saved tail to hardware tail
 *   3. For each completed descriptor: timestamp it, set sequence
 *      number, and call vb2_buffer_done() to return to userspace
 *   4. Update driver's saved tail
 *
 * In a real driver, this would also check for DMA errors, handle
 * buffer underruns, and potentially re-submit buffers.
 */
 ...
 * For NV12 video: 2 planes (Y + UV), with different sizes.
 */
 ...
 *
 * Called by VIDIOC_QBUF (and internally by VB2 before start_streaming).
 *
...
static int vcam_start_streaming(struct vb2_queue *q, unsigned int count)
{
	...
	/*
	 * Allocate the descriptor ring and make it visible to buf_queue.
	 * The spin_lock ensures buf_queue sees ring != NULL atomically
	 * with the ring being fully initialized.
	 */
	...
	/* Descriptor ring: array of vcam_hw_desc shared between driver and hardware */

	/* Register ring with hardware (resets HEAD/TAIL to 0) */
	ret = vcam_hw_set_buf_ring(vcam->ring, VCAM_RING_SIZE);
	if (ret)
		goto err_free_ring;
	/* Tell hardware where the ring lives -- equivalent to writing a DMA base address register */

	/* Drain the queued buffer list into the hardware ring AFTER
	 * vcam_hw_set_buf_ring, which resets HEAD/TAIL to zero.
	 */
	spin_lock_irqsave(&vcam->buf_lock, flags);
	...
	/* Configure the hardware's image pipeline -- real ISPs have dozens of format registers */

	/* Unmask frame-done interrupt */
	iowrite32((u32)~VCAM_INT_FRAME_DONE, vcam->regs + VCAM_INT_MASK);
	/* Unmask: write 0 to enable, 1 to disable (hardware convention from VNET) */

	/* Enable sensor and start streaming */
	iowrite32(VCAM_CTRL_ENABLE | VCAM_CTRL_STREAM_ON |
		  VCAM_CTRL_RING_ENABLE, vcam->regs + VCAM_CTRL);
	vcam_hw_notify_ctrl();
	/* Doorbell: this write starts the hardware's frame capture engine */
...
static void vcam_stop_streaming(struct vb2_queue *q)
{
	...
	vcam_hw_notify_ctrl();
	/* Kill the capture engine -- hardware stops generating frames immediately */
	...
	/* Ensure no bottom-half is running before we free the ring */
	...
	/* Buffers still on the driver list (shouldn't happen, but be safe) */
	list_for_each_entry_safe(buf, tmp, &vcam->buf_list, list) {
		list_del(&buf->list);
	...
	/* Tear down the DMA ring -- real drivers would also unmap DMA addresses */
	kfree(vcam->ring_bufs);
			 ...
			 struct v4l2_capability *cap)
{
	...
	strscpy(cap->card, "VCAM-2000 Capture IRQ", sizeof(cap->card));
	strscpy(cap->bus_info, "platform:vcam_irq", sizeof(cap->bus_info));
	return 0;
}
...
MODULE_DESCRIPTION("Part 5: V4L2 Capture with Interrupt-Driven Completion");
