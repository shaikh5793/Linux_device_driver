...
/*
 * Part 7: V4L2 Capture Device with EXPBUF
 *
 * Copyright (c) 2024 TECH VEDA
 * Author: Raghu Bharadwaj
 *
 * Concepts introduced (over Part 6):
 *   - VIDIOC_EXPBUF to export V4L2 buffers as dma-buf file descriptors
 *   - VB2_DMABUF io_mode flag enabling dma-buf operations
 *   - Zero-copy buffer sharing between kernel modules
 *
			 ...
			 struct v4l2_capability *cap)
{
	...
	strscpy(cap->card, "VCAM-2000 EXPBUF", sizeof(cap->card));
	strscpy(cap->bus_info, "platform:vcam_expbuf", sizeof(cap->bus_info));
	return 0;
}
...
static int vcam_probe(struct platform_device *pdev)
{
	...
	/* Setup VB2 queue — note VB2_DMABUF for EXPBUF support */
	q = &vcam->queue;
...
MODULE_DESCRIPTION("Part 7: V4L2 Capture Device with EXPBUF");
