#!/bin/bash
# Part 8: Kernel-Side DMA-BUF Import — Automated Test with Feature Demonstration
...
echo "============================================"
echo " Part 8: Kernel-Side DMA-BUF Import"
echo " NEW CONCEPT: Kernel-side dma_buf_get + vmap for buffer access"
echo "============================================"
echo ""
...
make -s 2>&1 | tail -1
gcc -Wall -o ../part7/test_expbuf ../part7/test_expbuf.c 2>&1 || true
echo ""
...
# --- Step 3: Run Test ---
echo "--- Test Output ---"
log_info "Running part7/test_expbuf to exercise buf_reader"
set +e
sudo ../part7/test_expbuf 2>&1
TEST_RC=$?
set -e
...
echo -e "${CYAN}=== Feature Demonstration: Kernel-Side DMA-BUF Import (dma_buf_get + vmap) ===${NC}"
echo ""
echo "  -- buf_reader misc device --"
ls -la /dev/buf_reader 2>/dev/null || log_info "/dev/buf_reader not found"
echo ""
if command -v v4l2-ctl &>/dev/null; then
    VDEV=$(find_vcam_dev "vcam_expbuf")
    if [ -n "$VDEV" ]; then
        log_info "Capture device (vcam_expbuf) at $VDEV"
        echo ""
        echo "  -- v4l2-ctl --all (capture device) --"
        v4l2-ctl -d "$VDEV" --all 2>&1 | head -20
    else
...
sudo dmesg | grep -i "buf_reader" | tail -15
echo ""
