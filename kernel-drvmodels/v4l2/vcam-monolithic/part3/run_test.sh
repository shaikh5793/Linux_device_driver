#!/bin/bash
# Part 3: VB2 MMAP Streaming — Automated Test with Feature Demonstration
...
cleanup() {
    echo ""
    echo "--- Cleanup ---"
    sudo rmmod vcam_vb2 2>/dev/null || true
...
echo "============================================"
echo " Part 3: VB2 MMAP Streaming"
echo " NEW CONCEPT: vb2_queue with MMAP memory type for zero-copy streaming"
echo "============================================"
echo ""
...
echo -e "${CYAN}=== Feature Demonstration: VB2 Buffer Management & MMAP Streaming ===${NC}"
if command -v v4l2-ctl &>/dev/null; then
    VDEV=$(find_vcam_dev "vcam_vb2")
    if [ -n "$VDEV" ]; then
        log_info "Found vcam_vb2 device at $VDEV"
        echo ""
        echo "  -- v4l2-ctl --all --"
        v4l2-ctl -d "$VDEV" --all 2>&1 | head -30
        echo ""
        echo "  -- v4l2-ctl --stream-mmap (3 frames) --"
        v4l2-ctl -d "$VDEV" --stream-mmap --stream-count=3 --stream-to=/dev/null 2>&1 || log_info "stream-mmap not supported or failed"
    else
        log_info "Could not find vcam_vb2 device via v4l2-ctl"
    fi
else
...
sudo dmesg | grep -i "vcam_vb2" | tail -15
echo ""
