...
/*
 * VSOC-3000 Virtual Camera Sensor -- Part 8: Media Controller Basics
 *
 * Copyright (c) 2024 TECH VEDA
 * Author: Raghu Bharadwaj
 *
 * NEW CONCEPT: media_entity with media_pad on a v4l2_subdev
 *
 * Builds on Part 7 by adding:
 *   - 1 source media_pad on the sensor entity
 *   - media_entity_pads_init() in probe
 *   - sd->entity.function = MEDIA_ENT_F_CAM_SENSOR
 *
 * The sensor is still an I2C subdev with async registration,
 * pad format ops, V4L2 controls, and s_stream from Part 7.
 *
 * Load order:
 *   1. insmod soc_hw_platform.ko
 *   2. insmod vsoc_bridge.ko
 *   3. insmod vsoc_sensor.ko   (async bound to bridge)
 */
...
struct vsoc_sensor {
	...
	struct media_pad	pad;		/* NEW: source pad */
...
static int vsoc_sensor_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		/* Write format to hardware before streaming */
...
/*
 * vsoc_sensor_probe -- Part 8: Media Controller Basics
 *
 * Step 1: Allocate and initialize driver state
 * Step 2: Verify hardware -- read chip ID via I2C
 * Step 3: [CHANGED] Set default format in driver struct (sensor->fmt) instead of state
 * Step 4: Initialize v4l2_subdev with all ops
 * Step 5: Initialize media entity with 1 source pad
 * Step 6: [CHANGED] Set entity function to MEDIA_ENT_F_CAM_SENSOR
 * Step 7: Initialize V4L2 controls
 * Step 8: Register with async framework
 *
 * Changes from Part 7:
 *   ~ Removed internal_ops / init_state (format stored in sensor->fmt directly)
 *   ~ Removed v4l2_subdev_init_finalize (no subdev state needed)
 *   ~ Removed hardware register writes at probe (deferred to s_stream)
 *   + Set MEDIA_ENT_F_CAM_SENSOR entity function for media controller
 *   Simplified: format lives in driver struct, not framework state.
 */
...
/*
 * vsoc_sensor_remove -- Part 8: Media Controller Basics
 *
...
MODULE_DESCRIPTION("VSOC-3000 Virtual Sensor -- Part 8: Media Controller Basics");
