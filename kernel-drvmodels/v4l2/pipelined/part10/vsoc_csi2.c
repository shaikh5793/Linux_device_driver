...
/*
 * VSOC-3000 CSI-2 Receiver Subdev -- Part 10: Pipeline Validation
 *
 * Copyright (c) 2024 TECH VEDA
 * Author: Raghu Bharadwaj
 *
 * NEW CONCEPT: .link_validate pad op for pipeline format checking
 *
 * Builds on Part 9 CSI-2 by adding:
 *   - .link_validate = v4l2_subdev_link_validate_default on the sink pad
 *     This checks that the source pad format (from the sensor) matches
 *     the sink pad format (on the CSI-2) in width, height, and mbus code.
 *
 * When media_pipeline_start() is called, the media framework walks all
 * enabled links and calls .link_validate on each sink pad.  If the
 * sensor and CSI-2 have mismatched formats, streaming will be refused.
 *
 * Load order:
...
static int vsoc_csi2_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable) {
		...
		iowrite32(4, csi2->regs + VSOC_CSI2_LANES);
...
/* ====================================================================
 * v4l2_subdev_pad_ops (format passthrough + link validation)
 * ==================================================================== */
 ...
 *
 * Unchanged from Part 9. Format passthrough: one format, both pads.
 */
...
static const struct v4l2_subdev_pad_ops vsoc_csi2_pad_ops = {
	...
	/*
	 * NEW: link_validate on the sink pad.
	 *
	 * v4l2_subdev_link_validate_default() compares the source pad
	 * format (from the upstream entity) with our sink pad format.
	 * If width, height, or mbus code differ, it returns -EPIPE.
	 *
	 * This is called automatically by media_pipeline_start() when
	 * it walks the enabled links in the pipeline.
	 */
...
static const struct media_entity_operations vsoc_csi2_entity_ops = {
...
/*
 * vsoc_csi2_probe — Part 10: Pipeline Validation
 *
 * Step 1-8: Same as Part 9
 * Step 9: [NEW] Set entity ops with link_validate support
 *
 * Changes from Part 9:
 *   + Added entity ops enabling media_pipeline_start() validation
 *   + pad_ops now includes .link_validate = v4l2_subdev_link_validate_default
 *   Default validator compares width, height, mbus code between pads.
 */
static int vsoc_csi2_probe(struct platform_device *pdev)
{
	...
	/* Step 9: Set entity ops with link_validate support */
	csi2->sd.entity.ops = &vsoc_csi2_entity_ops;
	...
	dev_info(&pdev->dev,
		 "vsoc_csi2 probed (2 pads, link_validate enabled)\n");
	return 0;
...
MODULE_DESCRIPTION("VSOC-3000 CSI-2 Receiver -- Part 10: Pipeline Validation");
