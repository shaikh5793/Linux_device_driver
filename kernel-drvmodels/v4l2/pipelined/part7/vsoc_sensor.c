...
/*
 * VSOC-3000 Virtual Camera Sensor — Part 7: Async Subdev Registration
 *
 * Copyright (c) 2024 TECH VEDA
 * Author: Raghu Bharadwaj
 *
 * NEW CONCEPT: v4l2_async_register_subdev()
 *
 * Builds on Part 6 but uses asynchronous subdev registration.
 * The sensor announces itself via v4l2_async_register_subdev(),
 * and the bridge's async notifier will discover it.
 *
 * This enables load-order independence:
 *   - sensor before bridge, OR bridge before sensor — both work.
 *
 * Load order (either works):
 *   1. insmod soc_hw_platform.ko
 *   2a. insmod vsoc_sensor.ko then insmod vsoc_bridge.ko
 *   2b. insmod vsoc_bridge.ko then insmod vsoc_sensor.ko
 */
 ...
 *
 * Key difference from Part 6: uses v4l2_async_register_subdev()
 * so the bridge can discover us via async notifier.
 * ==================================================================== */

/*
 * vsoc_sensor_probe -- Part 7: Async Subdev Registration
 *
 * Step 1: Allocate and initialize driver state
 * Step 2: Verify hardware -- read chip ID via I2C
 * Step 3: Initialize v4l2_subdev with all ops
 * Step 4: Set internal_ops (.init_state)
 * Step 5: Initialize media entity with 1 source pad
 * Step 6: Allocate subdev state via v4l2_subdev_init_finalize
 * Step 7: Initialize V4L2 controls
 * Step 8: Write default format to sensor registers
 * Step 9: [NEW] Register with async framework (v4l2_async_register_subdev)
 *         Sensor announces: "I exist, if anyone is looking for me"
 *
 * Changes from Part 6:
 *   + Added v4l2_async_register_subdev (Step 9) -- enables load-order independence
 */
static int vsoc_sensor_probe(struct i2c_client *client)
{
	...
	/* Step 9: Register with async framework (v4l2_async_register_subdev) */
	...
	dev_info(&client->dev,
		 "v4l2_subdev '%s' async-registered (waiting for bridge)\n",
		 sensor->sd.name);
	return 0;
}

/*
 * vsoc_sensor_remove -- Part 7: Async Subdev Registration
 *
 * Step 1: Recover driver state
 * Step 2: [NEW] Unregister from async framework (v4l2_async_unregister_subdev)
 * Step 3: Free control handler
 * Step 4: Clean up subdev state
 * Step 5: Clean up media entity
 *
 * Changes from Part 6:
 *   + Added v4l2_async_unregister_subdev (Step 2)
 */
static void vsoc_sensor_remove(struct i2c_client *client)
{
	...
	/* Step 2: Unregister from async framework (v4l2_async_unregister_subdev) */
...
MODULE_DESCRIPTION("VSOC-3000 Virtual Sensor — Part 7: Async Subdev Registration");
