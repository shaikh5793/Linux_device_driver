...
/*
 * VSOC-3000 Virtual Camera Bridge -- Part 9: Media Links & Topology
 *
 * Copyright (c) 2024 TECH VEDA
 * Author: Raghu Bharadwaj
 *
 * NEW CONCEPTS: media_create_pad_link, CSI-2 subdev integration
 *
 * Builds on Part 8 by adding:
 *   - CSI-2 subdev lookup via platform device driver data
 *   - v4l2_device_register_subdev() for CSI-2 (SoC-internal, not async)
 *   - Sink pad on the video_device entity
 *   - Media links: sensor:0 -> csi2:0 (ENABLED|IMMUTABLE)
 ...
 *   2. insmod vsoc_csi2.ko       (platform subdev)
 *   3. insmod vsoc_bridge.ko     (platform bridge)
 *   4. insmod vsoc_sensor.ko     (I2C sensor, async bound)
 */
...
struct vsoc_bridge {
	...
	struct media_pad	vid_pad;	/* NEW: sink pad for video node */
...
/*
 * vsoc_bridge_start_streaming — Part 9
 *
 * Step 1: Set streaming state and reset sequence
 * Step 2: [NEW] Start CSI-2 first: v4l2_subdev_call(csi2, s_stream, 1)
 * Step 3: Start sensor: v4l2_subdev_call(sensor, s_stream, 1)
 *         Order matters: receiver ready BEFORE transmitter
 * Step 4: Start frame simulation timer
 *
 * Changes from Part 8:
 *   + Added CSI-2 s_stream call (Step 2)
 *   ~ s_stream order: CSI-2 first, then sensor (downstream before upstream)
 */
static int vsoc_bridge_start_streaming(struct vb2_queue *vq, unsigned int count)
{
	...
	/* Step 2: Start CSI-2 first — s_stream(1) */
	if (bridge->csi2_sd) {
	...
	/* Step 3: Start sensor — s_stream(1), receiver ready before transmitter */
...
/*
 * vsoc_bridge_stop_streaming — Part 9
 *
 * Step 1: Stop streaming flag
 * Step 2: Stop frame timer
 * Step 3: [CHANGED] Stop sensor FIRST: s_stream(0)
 * Step 4: [NEW] Stop CSI-2: s_stream(0)
 *         Order: upstream first when stopping (reverse of start)
 * Step 5: Return pending buffers
 *
 * Changes from Part 8:
 *   + Added CSI-2 s_stream(0) (Step 4)
 *   ~ Stop order: sensor first, then CSI-2 (upstream before downstream)
 */
static void vsoc_bridge_stop_streaming(struct vb2_queue *vq)
{
	...
	/* Step 3: Stop sensor FIRST — s_stream(0) */
	v4l2_subdev_call(bridge->sensor_sd, video, s_stream, 0);
	/* Step 4: Stop CSI-2 — s_stream(0) */
	if (bridge->csi2_sd)
		v4l2_subdev_call(bridge->csi2_sd, video, s_stream, 0);
 ...
 *
 * The CSI-2 driver stores its struct vsoc_csi2 as platform driver data.
 * We use vsoc_hw_get_csi2_pdev() to find the platform device, then
 * retrieve the v4l2_subdev from the driver-private data.
 * ==================================================================== */
...
/*
 * vsoc_bridge_notify_complete — Part 9: Media Links & Topology
 *
 * Step 1: [NEW] Find CSI-2 subdev via platform helper
 * Step 2: [NEW] Register CSI-2 subdev with v4l2_device
 * Step 3: [NEW] Initialize video entity sink pad (media_entity_pads_init)
 * Step 4: Register video_device
 * Step 5: [NEW] Create media link: sensor:0 -> csi2:0 (ENABLED|IMMUTABLE)
 * Step 6: [NEW] Create media link: csi2:1 -> video:0 (ENABLED|IMMUTABLE)
 * Step 7: Register media_device
 *
 * Changes from Part 8:
 *   + CSI-2 subdev discovery and registration (Steps 1-2)
 *   + Video entity now has a sink pad (Step 3)
 *   + Media links connecting the 3-entity pipeline (Steps 5-6)
 *   Pipeline: [sensor] --link--> [csi2] --link--> [video]
 */
static int vsoc_bridge_notify_complete(struct v4l2_async_notifier *notifier)
{
	...
	/* Step 1: Find CSI-2 subdev via platform helper */
	/* Step 2: Register CSI-2 subdev with v4l2_device */
	...
	/* Step 3: Initialize video entity sink pad (media_entity_pads_init) */
	...
	/* Step 5: Create media link — sensor:0 -> csi2:0 (ENABLED|IMMUTABLE) */
	/* Step 6: Create media link — csi2:1 -> video:0 (ENABLED|IMMUTABLE) */
	if (bridge->csi2_sd) {
...
static int vsoc_bridge_probe(struct platform_device *pdev)
{
	...
	dev_info(&pdev->dev,
		 "vsoc_bridge probed (waiting for sensor async bind)\n");
	return 0;
...
MODULE_DESCRIPTION("VSOC-3000 Camera Bridge -- Part 9: Media Links & Topology");
